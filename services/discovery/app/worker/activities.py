...
from app.services import linkedin_scraper
from app.services.linkedin_scraper import RawJob, SearchConfig
from app.services.publisher import publish_cookie_expired, publish_jobs_discovered
...
async def deduplicate_activity(inp: DeduplicateInput) -> DeduplicateResult:
    ...
    try:
        ...
        new_urls: list[str] = []
        for job, is_member in zip(inp.raw_jobs, results):
            if not is_member:
                new_jobs.append(job)
