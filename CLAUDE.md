...
**⚠️ v0.2 re-scope (decided 2026-07-11, PRD v0.2) — CODE COMPLETE 2026-07-16, NOT YET DEPLOYED.** B2C self-registration, credential-free public-source job discovery (LinkedIn cookie flow removed), JD entry via URL/text/screenshot, email-only notifications, analytics module removed, dual LLM-key deployment modes (self-hosted BYO key vs hosted platform key). All of it is implemented and locally verified on `main` (batches 4a–4e), but **production still runs the previous scope** until the 4a–4e deploy lands — do not assume v0.2 features exist at `arnoldshang.com` yet.
...
5. **Production deploy of 4a–4e**: `deploy.sh`; server `.env.production` needs GOOGLE creds + strong `KEYCLOAK_ADMIN_API_SECRET` filled first; rerun keycloak-init on prod; owner human-tests registration email arrival + Google login round-trip
