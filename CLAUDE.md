...
**v0.2 re-scope (decided 2026-07-11, PRD v0.2) — DEPLOYED TO PRODUCTION 2026-07-17 (prod runs `f77f908`).** B2C self-registration, credential-free public-source job discovery (LinkedIn cookie flow removed), JD entry via URL/text/screenshot, email-only notifications, analytics module removed, dual LLM-key deployment modes (self-hosted BYO key vs hosted platform key). Batches 4a–4e plus the account-P0 and verify-email fixes are live; email goes out via Resend from `noreply@jobcopilot.arnoldshang.com`. **Not yet deployed: `b7c712a`** (platform-mode daily AI quota + BYO key validation) — CI/CD green, awaiting the next deploy.
...
6. **Deploy `b7c712a`** (per-tenant daily AI quota + BYO key validation): CI/CD green; run `deploy.sh`, then verify quota 429 + key-save validation on prod. Owner human-tests still pending: fresh registration email round-trip + Google login round-trip
