...
6. ~~Replace the fake `deploy` job in `.github/workflows/cd.yml`~~ ✅ code shipped; **inert until the owner adds the repo secrets + environment reviewer**. The deploy runs ON the server (`infra/scripts/remote-deploy.sh`), reached through a forced-command shim (`infra/scripts/jobcopilot-deploy`) that accepts only `deploy <40-hex-sha>` and refuses any SHA that is not an ancestor of `origin/main`. `deploy.sh` is now a thin client over the same entry point — one implementation, so the hand-run and automated paths cannot drift. Consequences worth knowing: **production secrets never enter GitHub** (`infra/.env` is server-owned; only a human `deploy.sh` run installs it, and CD has no way to read or replace it), and deploying commit X now ships **X's `infra/` config**, not the current tree's — so a `GIT_REF=<old>` rollback reverts compose/Caddyfile too. Needs from the owner: `DEPLOY_SSH_KEY` / `DEPLOY_HOST` / `DEPLOY_KNOWN_HOSTS` secrets, a required reviewer on the `production` environment, and the shim installed at `/usr/local/bin/jobcopilot-deploy`
