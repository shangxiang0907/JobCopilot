...
6. The shim is **host provisioning, not application code**, and is never auto-installed. Refreshing it as a side effect of every routine deploy would mean a security control changes silently and often — the opposite of deliberate, visible and rare. A deploy therefore *fails* when the installed shim differs from the deployed commit's copy, which forces the correct order: install the shim, then deploy the commit carrying it. (The earlier version of this ADR only warned, on the reasoning that failing would block deploying the very commit that updates the shim. That reasoning was wrong — it merely requires the two steps to happen in the right order.) Rolling back to an older shim uses the root-only `JOBCOPILOT_SHIM_DRIFT=allow` override.
7. Deploys are **monotonic by default**: a commit that is an ancestor of the revision already running is refused. Every other gate passes for an older commit — it is on `main`, its CD jobs are green, and the final revision check confirms the containers match what was *requested* — so a backward deploy reads as completely healthy. Two accidents reach it: approving a CD run that queued while production moved on (GitHub's concurrency group serializes CD runs but cannot see a manual `deploy.sh` at all), and running `deploy.sh` from a local `main` that is behind origin. Migrations are what make it expensive rather than merely surprising: `alembic upgrade head` never reverses, so old code meets a newer schema — v0.3's `is_active` → `is_default` rename is exactly that shape. Rollback stays supported but must be stated: `JOBCOPILOT_ALLOW_ROLLBACK=1`, or `ROLLBACK=1` through `deploy.sh`, which is deliberately not implied by `GIT_REF`.

8. Images are **signed at build time and verified at deploy time** (cosign keyless, GitHub OIDC). This closes the single window digest pinning cannot: the deploy asks the registry what the `<sha>` TAG points to, and whoever can write to the registry decides that answer — everything after resolution is content-addressed, that one step is not. The Fulcio certificate binds the signature to this repository and workflow, so an image we did not build cannot carry one. Verification happens *before* a digest is admitted to the compose pins. Note the placement rule this implies, since it looks inconsistent with (6): cosign lives in `server-setup.sh` and IS installed by provisioning, whereas the shim is not, because cosign is not policy — it is a dependency of a check that `remote-deploy.sh` already defines, and `remote-deploy.sh` is already repo-defined. The shim is the one enforcement point that must *not* be, since the repo is exactly what a compromised CI can rewrite. `remote-deploy.sh` additionally asserts a cosign major version rather than an exact one, so the precise pin has a single home in `server-setup.sh`.
...
6. 包装器属于**主机 provisioning，而非应用代码**，因此永远不会被自动安装。把它的刷新变成每次例行部署的副作用，意味着一个安全控制会静默且频繁地变更——恰与「刻意、可见、罕见」相反。因此当已安装的包装器与所部署提交内的副本不一致时，部署会*失败*，从而强制正确的顺序：先安装包装器，再部署携带它的提交。（本 ADR 的早期版本只是告警，理由是「失败会挡住那个更新包装器的提交本身」。该理由是错的——它只是要求这两步按正确顺序发生。）回滚到旧版包装器使用仅 root 可用的 `JOBCOPILOT_SHIM_DRIFT=allow` 覆盖开关。
7. 部署默认**单调**：若某提交是当前运行 revision 的祖先，则拒绝部署。对一个旧提交而言，其余闸门全都会放行——它在 `main` 上、CD job 全绿、最终版本核验也确认容器与所**请求**的提交一致——因此一次回退式部署在日志中看起来完全健康。有两种意外可以走到这一步：批准一个在生产已前进期间排队等待的 CD 运行（GitHub 并发组会串行化 CD 运行，却完全看不到手动执行的 `deploy.sh`），以及在落后于 origin 的本地 `main` 上执行 `deploy.sh`。真正让它代价高昂而非仅仅令人意外的是迁移：`alembic upgrade head` 从不反向，于是旧代码遇上新 schema——v0.3 的 `is_active` → `is_default` 改名正是这种形态。回滚仍然受支持，但必须明说：`JOBCOPILOT_ALLOW_ROLLBACK=1`，或经 `deploy.sh` 的 `ROLLBACK=1`（刻意不由 `GIT_REF` 隐含推导）。

8. 镜像在**构建时签名、部署时验签**（cosign 无密钥模式 + GitHub OIDC）。这关闭了 digest 钉死唯一无法覆盖的窗口：部署时要向 registry 询问 `<sha>` **标签**指向什么，而答案由能写入该 registry 的人决定——解析之后的一切都是内容寻址的，唯独这一步不是。Fulcio 证书把签名绑定到本仓库与本工作流，因此并非我们构建的镜像无法携带有效签名。验签发生在 digest 被纳入 compose 钉死**之前**。此处需说明一条看似与第 (6) 条矛盾的放置规则：cosign 写在 `server-setup.sh` 中、**确实**由 provisioning 安装，而包装器不是——因为 cosign 不是策略，它只是 `remote-deploy.sh` 所定义的那道检查的依赖，而 `remote-deploy.sh` 本就由仓库定义。包装器是唯一**不能**由仓库定义的执行点，因为仓库恰恰是被攻破的 CI 能改写的东西。此外 `remote-deploy.sh` 只断言 cosign 的主版本而非精确版本，使精确 pin 在 `server-setup.sh` 中只有一处归属。
